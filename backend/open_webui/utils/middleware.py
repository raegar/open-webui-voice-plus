...
async def chat_image_generation_handler(
    ...
    if len(input_images) > 0 and request.app.state.config.ENABLE_IMAGE_EDIT:
        ...
        if request.app.state.config.ENABLE_IMAGE_PROMPT_GENERATION:
            try:
                image_gen_messages = form_data["messages"]
                log.info(f"[IMAGE_GEN_DEBUG] Sending {len(image_gen_messages)} messages to generate_image_prompt")
                for i, msg in enumerate(image_gen_messages[-4:]):
                    role = msg.get("role", "unknown")
                    content = msg.get("content", "")
                    if isinstance(content, list):
                        content = " ".join(c.get("text", "") for c in content if c.get("type") == "text")
                    log.info(f"[IMAGE_GEN_DEBUG] msg[-{4-i}] role={role} content={content[:200]}")
                ...
                try:
                    ...
                    log.info(f"[IMAGE_GEN_DEBUG] Final prompt sent to A1111: {prompt}")
                except Exception as e:
                    prompt = user_message
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if features:
        ...
        if "image_generation" in features and features["image_generation"]:
            # PATCHED: deferred to background_tasks_handler after response completes # Skip forced image generation when native FC is enabled - model can use generate_image tool
            #if metadata.get("params", {}).get("function_calling") != "native":
                ...
                if TASKS.TAGS_GENERATION in tasks and tasks[TASKS.TAGS_GENERATION]:
                    ...
                    if res and isinstance(res, dict):
                        ...
                        try:
                            ...
                            pass
        # PATCHED: Deferred image generation - runs after assistant response
        # is complete so generate_image_prompt has full context from both
        # the user message and the completed assistant response
        log.warning(f"[IMAGE_GEN_DEBUG] background_tasks_handler reached, features={metadata.get('features', {})}")
        log.warning(f"[IMAGE_GEN_DEBUG] messages count={len(messages)}, last 2 roles={[m.get('role') for m in messages[-2:]]}")
        for i, msg in enumerate(messages[-2:]):
            content = msg.get("content", "")
            if isinstance(content, list):
                content = " ".join(c.get("text", "") for c in content if c.get("type") == "text")
            log.warning(f"[IMAGE_GEN_DEBUG] messages[-{2-i}] role={msg.get('role')} content={str(content)[:300]}")
        if (
